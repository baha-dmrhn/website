...
# EPİAŞ yanıtları istek gövdesi + servis adresiyle anahtarlanıp JSON olarak
# saklanır. Güncel veri kontrollü aralıklarla yenilenir; tamamlanmış geçmiş
# veriler ise yeniden indirilmemesi için pratikte kalıcı tutulur.
DEFAULT_EPIAS_JSON_CACHE_CURRENT_SECONDS = 15 * 60
DEFAULT_EPIAS_JSON_CACHE_LIVE_SECONDS = 5 * 60
DEFAULT_EPIAS_JSON_CACHE_RECENT_SECONDS = 6 * 60 * 60
DEFAULT_EPIAS_JSON_CACHE_HISTORY_SECONDS = 10 * 365 * 24 * 60 * 60
DEFAULT_EPIAS_FORCE_REFRESH_MIN_SECONDS = 5 * 60
...
def last_complete_month_range(today: date | None = None) -> DateRange:
    """Return the first and last day of the latest fully completed month."""

    current_day = today or datetime.now(TR_TZ).date()
