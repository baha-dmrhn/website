...
public class MainActivity extends Activity {
    ...
    private static final String CONFIGURE_EMBEDDED_CHROME_SCRIPT =
            "(function(){"
                    ...
                    + ".baha-suite-sistem .suite-sidebar"
                    + "{padding:34px 20px 24px!important}"
                    + ".baha-suite-sistem .suite-side-brand"
                    + "{min-height:58px!important;gap:13px!important;"
                    + "padding:0 8px 36px!important;font-size:18px!important}"
                    + ".baha-suite-sistem .suite-sidebar nav"
                    + "{display:grid!important;height:auto!important;"
                    + "flex:0 0 auto!important;align-content:start!important;"
                    + "grid-auto-rows:48px!important;gap:8px!important}"
                    + ".baha-suite-sistem .suite-sidebar nav a"
                    + "{height:48px!important;min-height:48px!important;"
                    + "max-height:48px!important;margin:0!important;"
    ...
    private void buildUi() {
        ...
        appMenuSpacer = new View(this);
        appMenuSpacer.setVisibility(View.GONE);
        appLayout.addView(appMenuSpacer, new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                dp(64)
        ));
        ...
        appMenuButton.setVisibility(View.INVISIBLE);
        ...
        menuParams.leftMargin = dp(14);
        menuParams.topMargin = dp(12);
        root.addView(appMenuButton, menuParams);
    ...
    private void updateNativeChrome(String url) {
        ...
        if (appMenuButton != null) {
            appMenuButton.setContentDescription(title + " menüsü");
            appMenuButton.setVisibility(
                    selectedTabIndex >= 0 ? View.VISIBLE : View.INVISIBLE
            );
        }
        if (appMenuSpacer != null) {
            appMenuSpacer.setVisibility(
