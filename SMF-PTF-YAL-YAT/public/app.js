...
$('logout').onclick=async()=>{await api('/api/logout',{method:'POST'}).catch(()=>{});if(!desktopSidebar.matches)closeMenu();show('login')};$('refresh-button').onclick=()=>loadData(true);$('date-input').onchange=()=>{constrainMarketDate(true);loadData()};$('prev-date').onclick=()=>shiftDate(-1);$('next-date').onclick=()=>shiftDate(1);$('today-button').onclick=()=>{$('date-input').value=todayTR();constrainMarketDate();loadData()};
