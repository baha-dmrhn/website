...
let nextDayPtfRefreshTimer = 0, marketRefreshTimer = 0;
...
let priceChartMode = localStorage.getItem('baha-market-chart-mode') || 'full';
const SUITE_FONT = 'DM Sans, Inter, system-ui, sans-serif';
const API_BASE_URL = String(window.BAHA_CONFIG?.apiBaseUrl || '').replace(/\/$/, '');
const DATA_CACHE_PREFIX='baha-market-v6:';
let chartEngineLoadPromise=null;
function loadChartEngineScript(src){return new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=src;script.dataset.chartEngine='fallback';script.onload=()=>window.ApexCharts?resolve():reject(new Error('Grafik motoru yüklenemedi.'));script.onerror=reject;document.head.appendChild(script)})}
function ensureChartEngine(){if(window.ApexCharts)return Promise.resolve();if(chartEngineLoadPromise)return chartEngineLoadPromise;const existing=document.querySelector('script[data-chart-engine]')?.getAttribute('src');const candidates=[...new Set([existing,'/piyasa-charts.js?v=19','piyasa-charts.js?v=19'].filter(Boolean))];chartEngineLoadPromise=(async()=>{for(const src of candidates){try{await loadChartEngineScript(src);if(window.ApexCharts)return}catch{}}chartEngineLoadPromise=null;throw new Error('Grafik motoru yüklenemedi.')})();return chartEngineLoadPromise}
function renderMarketCharts(){return ensureChartEngine().then(()=>{renderChart();renderOperationalCharts()}).catch(error=>{const message=error.message||'Grafik motoru yüklenemedi.';setMarketEmptyState('price-chart',message);setMarketEmptyState('quantity-chart',message);setMarketConnectionPart('main','warning',message);alertAt('data-alert',message)})}
const CURRENCIES={
  TRY:{label:'TL',unit:'TL/MWh',icon:'₺'},
  EUR:{label:'EUR',unit:'EUR/MWh',icon:'€'},
  USD:{label:'USD',unit:'USD/MWh',icon:'$'}
};
const initialUrlState=readMarketUrlState();
if(!['full','simple','ptf'].includes(priceChartMode))priceChartMode='full';
function readMarketUrlState(){const params=new URLSearchParams(window.location.search);const currency=params.get('currency');const view=params.get('view');if(currency&&CURRENCIES[currency]){currentCurrency=currency;localStorage.setItem('baha-market-currency',currency)}if(view&&['full','simple','ptf'].includes(view)){priceChartMode=view;localStorage.setItem('baha-market-chart-mode',view)}return{date:params.get('date')||'',compare:params.get('compare')||'',compareDate:params.get('compareDate')||'',currency:currency||'',view:view||''}}
function syncMarketUrl(){if(!currentDate||!window.history?.replaceState)return;const params=new URLSearchParams();const compareMode=$('compare-select')?.value||'day';const compareDate=$('compare-date-input')?.value||'';if(currentDate!==todayTR())params.set('date',currentDate);if(currentCurrency!=='TRY')params.set('currency',currentCurrency);if(compareMode&&compareMode!=='day')params.set('compare',compareMode);if(compareMode==='custom'&&compareDate)params.set('compareDate',compareDate);if(priceChartMode!=='full')params.set('view',priceChartMode);const query=params.toString();window.history.replaceState(null,'',`${window.location.pathname}${query?`?${query}`:''}${window.location.hash||''}`)}
function applyInitialMarketUrlState(){const today=todayTR(),dateInput=$('date-input'),compareSelect=$('compare-select'),compareInput=$('compare-date-input');dateInput.value=initialUrlState.date&&initialUrlState.date<=today?initialUrlState.date:today;if(initialUrlState.compare&&Array.from(compareSelect.options).some(option=>option.value===initialUrlState.compare))compareSelect.value=initialUrlState.compare;toggleCompareDateInput();if(initialUrlState.compare==='custom'&&initialUrlState.compareDate){compareInput.value=initialUrlState.compareDate;constrainCompareDate()}constrainMarketDate()}
function updateChartModeUi(){document.querySelectorAll('[data-chart-mode]').forEach(button=>{const selected=button.dataset.chartMode===priceChartMode;button.classList.toggle('active',selected);button.setAttribute('aria-pressed',String(selected))})}
...
function hasPendingSmf(data,date=data?.date){return data?.freshness?.smfIncomplete||((data?.rows||[]).some(row=>isSmfUnpublished(row))&&dateIsToday(date))}
function dateIsToday(date){return date===todayTR()}
function marketCacheTtl(date,data){const suggested=Number(data?.freshness?.clientCacheMs);if(Number.isFinite(suggested)&&suggested>0)return suggested;return date===todayTR()?(hasPendingSmf(data,date)?15*1000:2*60*1000):7*24*60*60*1000}
function writeDataCache(date,data){try{const ttl=marketCacheTtl(date,data);localStorage.setItem(DATA_CACHE_PREFIX+date,JSON.stringify({expires:Date.now()+ttl,data}))}catch{}}
...
function priceValues(rows,key){return rows.filter(row=>key!=='smf'||!isSmfUnpublished(row)).map(row=>priceValue(row,key)).filter(Number.isFinite)}
...
function constrainCompareDate(){const input=$('compare-date-input'),today=todayTR();if(!input) return;input.max=today;let corrected=false;if(input.value>today){input.value=today;corrected=true}if(corrected)alertAt('data-alert','Karşılaştırma için geçerli bir tarih seçin.');return corrected}
function toggleCompareDateInput(){const compareMode=$('compare-select').value;const input=$('compare-date-input');if(!input)return;const isCustom=compareMode==='custom';input.classList.toggle('d-none',!isCustom);if(isCustom){if(!input.value){const current=$('date-input').value||todayTR();const date=new Date(`${current}T12:00:00`);date.setDate(date.getDate()-1);input.value=date.toISOString().slice(0,10);}constrainCompareDate()}}
...
function comparisonDate(){const mode=$('compare-select').value;if(mode==='none'){compareDateValue='';compareLabel='';return null}if(mode==='custom'){const custom=$('compare-date-input').value;compareDateValue=custom;compareLabel=custom?shortDate(custom):'';return custom||null}if(mode==='year-weekday'){compareDateValue=previousYearSameWeekday(currentDate);compareLabel=`${shortDate(compareDateValue)} · aynı hafta/gün`;return compareDateValue}const date=new Date(`${currentDate}T12:00:00`);date.setDate(date.getDate()-(mode==='week'?7:1));compareDateValue=date.toISOString().slice(0,10);compareLabel=mode==='week'?'7 gün önce':'Önceki gün';return compareDateValue}
function comparisonPhrase(){const mode=$('compare-select').value;if(mode==='custom')return compareDateValue?'seçilen tarihe':'seçilen tarihe';return mode==='year-weekday'?'önceki yılın aynı hafta ve gününe':mode==='week'?'7 gün öncesine':'önceki güne'}
...
function renderTable(loading=false) {
  $('data-body').innerHTML = loading ? Array.from({length:8},()=>`<tr>${'<td><div class="skeleton"></div></td>'.repeat(7)}</tr>`).join('') : currentRows.map(r=>{
    ...
    const yalYat = Number.isFinite(r.yal) ? r.yal - (Number.isFinite(r.yat) ? Math.abs(r.yat) : 0) : null;
    return `<tr><td><b>${esc(r.time)}</b></td><td>${fmt(priceValue(r,'ptf'))}</td><td>${fmt(isSmfUnpublished(r)?null:priceValue(r,'smf'))}</td><td>${fmt(r.yal)}</td><td>${fmt(r.yat)}</td><td>${fmt(yalYat)}</td><td><span class="direction ${cls}">${esc(direction)}</span></td></tr>`;
  }).join('') || '<tr><td colspan="7" class="text-center text-secondary py-5">Bu tarih için veri bulunamadı.</td></tr>';
}
function isSmfUnpublished(row){if(typeof row?.smfPublished==='boolean')return !row.smfPublished;return !Number.isFinite(row?.smf)||Number(row.smf)===0}
function publishedSmfValue(row){return isSmfUnpublished(row)?null:priceValue(row,'smf')}
function lastPublishedSmfIndex(rows=currentRows){return rows.reduce((last,row,index)=>isSmfUnpublished(row)?last:index,-1)}
function isTrailingUnpublishedSmf(row,index,rows=currentRows){const last=lastPublishedSmfIndex(rows);return last>=0&&index>last&&isSmfUnpublished(row)}
function firstTrailingUnpublishedSmfIndex(rows=currentRows){const last=lastPublishedSmfIndex(rows);return last>=0&&rows[last+1]&&isSmfUnpublished(rows[last+1])?last+1:-1}
function smfBridgeValue(row,index,rows=currentRows){const first=firstTrailingUnpublishedSmfIndex(rows),last=lastPublishedSmfIndex(rows);if(first<0)return null;if(index===last)return priceValue(row,'smf');if(index===first)return 0;return null}
function unpublishedSmfValue(row,index,rows=currentRows){return isTrailingUnpublishedSmf(row,index,rows)?0:null}
function renderChart() {
  const foreignCurrency=currentCurrency!=='TRY';
  const showSmf=!foreignCurrency&&priceChartMode!=='ptf';
  const showComparison=priceChartMode==='full'&&compareRows.length;
  const hasUnpublishedSmf=priceChartMode==='full'&&!foreignCurrency&&currentRows.some((row,index)=>isTrailingUnpublishedSmf(row,index,currentRows));
  const series=[{name:'PTF',data:currentRows.map(r=>priceValue(r,'ptf')),fill:false}];
  const colors=['#2c70f4'];
  const strokeWidths=[2.4];
  const dashArray=[0];
  if(showSmf){
    series.push({name:'SMF',data:currentRows.map(publishedSmfValue),fill:false});
    colors.push('#8a67e8');strokeWidths.push(2.4);dashArray.push(0);
  }
  if(showComparison){
    series.push({name:`PTF · ${compareLabel}`,data:compareRows.map(r=>priceValue(r,'ptf')),fill:false});
    colors.push('#91b5fa');strokeWidths.push(1.7);dashArray.push(6);
    if(showSmf){
      series.push({name:`SMF · ${compareLabel}`,data:compareRows.map(r=>priceValue(r,'smf')),fill:false});
      colors.push('#b9a8ef');strokeWidths.push(1.7);dashArray.push(6);
    }
  }
  if(hasUnpublishedSmf){
    series.push({name:'SMF geçiş',data:currentRows.map(smfBridgeValue),fill:false,hideMarkers:true,hideInTooltip:true,hideInLegend:true});
    colors.push('#8a67e8');strokeWidths.push(2.4);dashArray.push(0);
    series.push({name:'SMF yayınlanmadı',data:currentRows.map(unpublishedSmfValue),fill:false,hideMarkers:true,hideNullInTooltip:true,hideInTooltip:true});
    colors.push('#d5573b');strokeWidths.push(2.2);dashArray.push(0);
  }
  const missingKey=$('smf-missing-chart-key');
  if(missingKey)missingKey.classList.toggle('d-none',!hasUnpublishedSmf);
  const markers={size:0,hover:{size:5}};
  const options={chart:{type:'line',height:320,toolbar:{show:false},zoom:{enabled:false},selection:{enabled:false},fontFamily:SUITE_FONT,events:{click:(event,ctx,config)=>{if(config.dataPointIndex>=0)showHourDetail(config.dataPointIndex)},dataPointSelection:(event,ctx,config)=>showHourDetail(config.dataPointIndex)}},theme:{mode:document.documentElement.dataset.theme},series,colors,stroke:{curve:'smooth',width:strokeWidths,dashArray},markers,fill:{opacity:0},dataLabels:{enabled:false},xaxis:{categories:currentRows.map(r=>r.time),tickAmount:8,labels:{style:{colors:'#8994a6',fontSize:'10px'}}},yaxis:{labels:{formatter:v=>fmt(v),style:{colors:'#8994a6',fontSize:'10px'}}},grid:{borderColor:document.documentElement.dataset.theme==='dark'?'#293750':'#edf0f4',strokeDashArray:3},legend:{show:showComparison||hasUnpublishedSmf,position:'bottom',fontSize:'11px'},tooltip:{shared:true,intersect:false,y:{formatter:v=>`${fmt(v)} ${currencyUnit()}`}},noData:{text:'Veri bulunamadı'}};
...
function renderNextDayPtf(data={}){
  ...
  if(!pricedRows.length){
    ...
    status.className=nextDayPtfData.published?'currency-missing':'waiting';status.innerHTML=`<i></i> ${nextDayPtfData.published?`${esc(CURRENCIES[displayCurrency].label)} verisi yok`:esc(nextDayPtfData.publication?.label||'Henüz yayımlanmadı')}`;
  ...
  if(!window.ApexCharts){$('next-day-ptf-chart').innerHTML='<div class="next-day-ptf-empty">Grafik motoru yükleniyor…</div>';ensureChartEngine().then(()=>renderNextDayPtf(nextDayPtfData)).catch(error=>{$('next-day-ptf-chart').innerHTML=`<div class="next-day-ptf-empty">${esc(error.message||'Grafik motoru yüklenemedi.')}</div>`});return}
...
async function loadData(force=false){
  ...
  try{
    ...
    scheduleMarketDataRefresh(data,sequence);
...
const desktopSidebar=window.matchMedia('(min-width:821px)'),desktopLock=window.matchMedia('(min-width:1025px) and (hover:hover) and (pointer:fine)'),sidebarStorageKey='baha-sidebar-collapsed',sidebarPinnedStorageKey='baha-sidebar-pinned';
let desktopSidebarHoverTimer=0,desktopSidebarPointerInside=false;
function isSidebarPinned(){return localStorage.getItem(sidebarPinnedStorageKey)==='true'}
function canPinSidebar(){return desktopLock.matches}
function syncSidebarLockUi(pinned=isSidebarPinned()){const locked=canPinSidebar()&&pinned;document.body.classList.toggle('suite-sidebar-pinned',locked);if(sidebarLock){sidebarLock.classList.toggle('is-locked',locked);sidebarLock.setAttribute('aria-pressed',String(locked));const label=locked?'Yan menü kilidini aç':'Yan menüyü sabitle';sidebarLock.setAttribute('aria-label',label);sidebarLock.title=label}}
function setMenu(open,persist=true){if(desktopSidebar.matches){if(open)document.body.classList.remove('suite-sidebar-dismissed');const collapsed=!open;sidebar.classList.remove('open');document.body.classList.remove('sidebar-open');if(!open)document.body.classList.remove('suite-sidebar-hovered');document.body.classList.toggle('suite-sidebar-collapsed',collapsed);$('menu-button').setAttribute('aria-expanded',String(open));if(persist)localStorage.setItem(sidebarStorageKey,String(collapsed));syncSidebarLockUi(open&&isSidebarPinned());return}document.body.classList.remove('suite-sidebar-collapsed','suite-sidebar-hovered','suite-sidebar-pinned','suite-sidebar-dismissed');sidebar.classList.toggle('open',open);document.body.classList.toggle('sidebar-open',open);$('menu-button').setAttribute('aria-expanded',String(open));syncSidebarLockUi(false)}
function setSidebarPinned(pinned){if(!canPinSidebar()){syncSidebarLockUi(false);if(desktopSidebar.matches)setMenu(false);return}localStorage.setItem(sidebarPinnedStorageKey,String(pinned));syncSidebarLockUi(pinned);if(desktopSidebar.matches)setMenu(pinned)}
function setDesktopSidebarHover(open){if(!desktopSidebar.matches||document.body.classList.contains('suite-sidebar-pinned')||(open&&document.body.classList.contains('suite-sidebar-dismissed'))||!document.body.classList.contains('suite-sidebar-collapsed'))return;window.clearTimeout(desktopSidebarHoverTimer);const apply=()=>{document.body.classList.toggle('suite-sidebar-hovered',open);$('menu-button').setAttribute('aria-expanded',String(open))};if(open)apply();else desktopSidebarHoverTimer=window.setTimeout(apply,120)}
function closeMenu(){if(desktopSidebar.matches){document.body.classList.add('suite-sidebar-dismissed');document.body.classList.remove('suite-sidebar-hovered');if(sidebar.contains(document.activeElement))document.activeElement.blur()}setDesktopSidebarHover(false);if(desktopSidebar.matches&&canPinSidebar()&&isSidebarPinned())setSidebarPinned(false);else setMenu(false)}
function syncMenuMode(){if(desktopSidebar.matches)setMenu(canPinSidebar()&&isSidebarPinned(),false);else setMenu(false,false)}
sidebar.addEventListener('mouseenter',()=>{desktopSidebarPointerInside=true;setDesktopSidebarHover(true)});
sidebar.addEventListener('mouseleave',()=>{desktopSidebarPointerInside=false;document.body.classList.remove('suite-sidebar-dismissed');setDesktopSidebarHover(false)});
...
$('menu-button').onclick=()=>{if(desktopSidebar.matches&&canPinSidebar())setSidebarPinned(true);else if(desktopSidebar.matches)setMenu(document.body.classList.contains('suite-sidebar-collapsed'));else setMenu(!sidebar.classList.contains('open'))};
$('menu-close').onclick=closeMenu;
if(sidebarLock)sidebarLock.onclick=()=>{if(desktopSidebar.matches&&canPinSidebar())setSidebarPinned(!isSidebarPinned())};
...
window.addEventListener('storage',event=>{if(event.key===sidebarPinnedStorageKey)syncMenuMode()});
desktopSidebar.addEventListener('change',syncMenuMode);desktopLock.addEventListener('change',syncMenuMode);syncMenuMode();
$('compare-select').onchange=()=>{toggleCompareDateInput();loadData();};
$('compare-date-input').onchange=()=>{constrainCompareDate();loadData();};
document.querySelectorAll('[data-currency]').forEach(button=>button.addEventListener('click',()=>{if(button.disabled||button.dataset.currency===currentCurrency)return;currentCurrency=button.dataset.currency;localStorage.setItem('baha-market-currency',currentCurrency);syncMarketUrl();if(currentRows.length)renderDashboard(currentData,comparisonData);else updateCurrencyUi(currentData.currencyInfo);if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)}));
document.querySelectorAll('[data-chart-mode]').forEach(button=>button.addEventListener('click',()=>{if(button.dataset.chartMode===priceChartMode)return;priceChartMode=button.dataset.chartMode;localStorage.setItem('baha-market-chart-mode',priceChartMode);syncMarketUrl();updateChartModeUi();if(currentRows.length)renderMarketCharts();updateCurrencyUi(currentData.currencyInfo)}));
$('theme-toggle').onclick=()=>{const theme=document.documentElement.dataset.theme==='dark'?'light':'dark';localStorage.setItem('baha-theme',theme);applyTheme(theme);if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)};
applyTheme(selectedTheme());
systemDark.addEventListener('change',()=>{if(!localStorage.getItem('baha-theme')){applyTheme(selectedTheme());if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)}});
window.addEventListener('baha:themechange',event=>{const theme=event.detail?.theme||selectedTheme();applyTheme(theme);if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)});
...
const marketDownloadMenu=document.querySelector('[data-download-menu="market"]');
const marketDownloadTrigger=$('xlsx-button');
const marketExportPaths={xlsx:'/api/export.xlsx?date=',pdf:'/api/export.pdf?date=',csv:'/api/export.csv?date='};
function closeMarketDownloadMenu(){marketDownloadMenu?.classList.remove('open');marketDownloadTrigger?.setAttribute('aria-expanded','false')}
marketDownloadTrigger.onclick=event=>{event.stopPropagation();if(!currentDate||!currentRows.length)return;const open=!marketDownloadMenu.classList.contains('open');marketDownloadMenu.classList.toggle('open',open);marketDownloadTrigger.setAttribute('aria-expanded',String(open))};
marketDownloadMenu?.querySelectorAll('[data-download-format]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();if(!currentDate||!currentRows.length)return;const format=button.dataset.downloadFormat||'xlsx';closeMarketDownloadMenu();window.location.href=`${marketExportPaths[format]||marketExportPaths.xlsx}${encodeURIComponent(currentDate)}`}));
document.addEventListener('click',closeMarketDownloadMenu);
document.addEventListener('keydown',event=>{if(event.key==='Escape')closeMarketDownloadMenu()});
(async()=>{try{const s=await api('/api/session');setUser(s);show('app');applyInitialMarketUrlState();loadData()}catch{window.location.replace('/login')}})();
