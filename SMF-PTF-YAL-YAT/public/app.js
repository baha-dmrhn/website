...
function renderTable(loading=false) {
  $('data-body').innerHTML = loading ? Array.from({length:8},()=>`<tr>${'<td><div class="skeleton"></div></td>'.repeat(7)}</tr>`).join('') : currentRows.map(r=>{
    ...
    return `<tr><td><b>${r.time}</b></td><td>${fmt(priceValue(r,'ptf'))}</td><td>${fmt(priceValue(r,'smf'))}</td><td>${fmt(r.yal)}</td><td>${fmt(r.yat)}</td><td>${fmt(yalYat)}</td><td><span class="direction ${cls}">${direction}</span></td></tr>`;
...
$('xlsx-button').onclick=()=>{if(!currentRows.length)return;if(!window.XLSX){alertAt('data-alert','Excel oluşturma bileşeni yüklenemedi. İnternet bağlantınızı kontrol edin.');return}const rows=[['Tarih','Saat','PTF (TL/MWh)','SMF (TL/MWh)','YAL (MWh)','YAT (MWh)','YAL - YAT (MWh)','Sistem Yönü'],...currentRows.map(r=>[currentDate,r.time,r.ptf,r.smf,r.yal,Number.isFinite(r.yat)?Math.abs(r.yat):null,Number.isFinite(r.yal)?r.yal-(Number.isFinite(r.yat)?Math.abs(r.yat):0):null,r.direction||''])];const sheet=XLSX.utils.aoa_to_sheet(rows);sheet['!cols']=[{wch:13},{wch:9},{wch:16},{wch:16},{wch:14},{wch:14},{wch:18},{wch:22}];for(let row=2;row<=rows.length;row++)for(const column of ['C','D','E','F','G'])if(sheet[`${column}${row}`])sheet[`${column}${row}`].z='#,##0.00';sheet['!autofilter']={ref:`A1:H${rows.length}`};const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,sheet,'Saatlik Veriler');XLSX.writeFile(book,`baha-enerji-${currentDate}.xlsx`,{compression:true})};
