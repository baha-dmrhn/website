...
const DATA_CACHE_PREFIX='baha-market-v5:';
...
function renderTable(loading=false) {
  $('data-body').innerHTML = loading ? Array.from({length:8},()=>`<tr>${'<td><div class="skeleton"></div></td>'.repeat(7)}</tr>`).join('') : currentRows.map(r=>{
    ...
    return `<tr><td><b>${esc(r.time)}</b></td><td>${fmt(priceValue(r,'ptf'))}</td><td>${fmt(priceValue(r,'smf'))}</td><td>${fmt(r.yal)}</td><td>${fmt(r.yat)}</td><td>${fmt(yalYat)}</td><td><span class="direction ${cls}">${esc(direction)}</span></td></tr>`;
  }).join('') || '<tr><td colspan="7" class="text-center text-secondary py-5">Bu tarih için veri bulunamadı.</td></tr>';
}
function isSmfUnpublished(row){return !Number.isFinite(row?.smf)||Number(row.smf)===0}
...
function setDesktopSidebarHover(open){if(!desktopSidebar.matches||document.body.classList.contains('suite-sidebar-pinned')||!document.body.classList.contains('suite-sidebar-collapsed'))return;window.clearTimeout(desktopSidebarHoverTimer);const apply=()=>{document.body.classList.toggle('suite-sidebar-hovered',open);$('menu-button').setAttribute('aria-expanded',String(open))};if(open)apply();else desktopSidebarHoverTimer=window.setTimeout(apply,120)}
function closeMenu(){setDesktopSidebarHover(false);if(desktopSidebar.matches&&canPinSidebar()&&isSidebarPinned())setSidebarPinned(false);else setMenu(false)}
