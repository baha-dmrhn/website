...
Her başarılı EPİAŞ veri cevabı servis adresi ve istek parametrelerine göre
anahtarlanıp `.epias-json-cache` altında JSON olarak saklanır. Sayfa yenilemek veya
başka bir oturumdan aynı tarih ve veri türünü açmak yeni token ya da veri isteği
oluşturmaz; kayıt doğrudan bu ortak JSON önbellekten okunur. Güncel günün henüz
tamamlanmamış verileri 5 dakikada, tarihi olmayan güncel servisler 15 dakikada,
dünkü veriler 6 saatte bir yenilenebilir. Daha eski tamamlanmış veriler 10 yıl
saklanarak pratikte kalıcı arşiv gibi kullanılır. Elle yenileme de aynı veri için
5 dakikadan sık EPİAŞ isteği gönderemez. JSON dosyalarında parola, TGT veya ST
bulunmaz.
...
5 dakika, ertesi gün PTF ve tüketim için 10 dakika, üretim için 60 dakikadır.
Baraj Aktif verisi Türkiye saatiyle 08.00-19.00 arasında her saat başında bir kez
kontrol edilir; 19.00 son kontroldür. Bu pencere
`BAHA_BG_BARAJ_START_HOUR` ve `BAHA_BG_BARAJ_END_HOUR` ile değiştirilebilir.
Durumunu `/epias-koruma` sayfasından izleyebilirsiniz.

Baraj Aktif servisi arşivdeki son tarihten daha yeni ve en az 50 geçerli baraj
kaydı içeren bir EPİAŞ günü yayımladığında, bu gün
`Aktif_Doluluk-Delta - Kopya.xlsx` dosyasının `Aktif Doluluk` ve `Pivot`
sekmelerine otomatik olarak eklenir. EPİAŞ cevabı birden fazla gün içerse bile
yalnızca en yeni yayımlanan gün alınır. Mevcut Excel tarihleri kilitli arşivdir;
aynı tarih yeniden yazılmaz ve eski eksik günler sonradan değiştirilmez. Excel yazımı HTTP yanıtını bekletmeyen arka plan
işinde ve geçici dosya üzerinden atomik yapılır. Gerekirse
`BAHA_BARAJ_ARCHIVE_AUTO_UPDATE=false` ile kapatılabilir veya tam gün eşiği
`BAHA_BARAJ_ARCHIVE_MIN_RECORDS` ile değiştirilebilir.

Render'ın ücretsiz servis dosya sistemi yeni deploy veya instance değişiminde
silinebilir. JSON arşivinin deploy'lar arasında da korunması istenirse
`BAHA_EPIAS_JSON_CACHE_DIR` kalıcı bir Render disk yoluna verilmelidir. Kalıcı disk
olmadan önbellek, çalışan instance boyunca sayfa yenilemelerini ve tekrarlı
istekleri engeller.
Excel'e yapılan otomatik baraj eklemeleri de ücretsiz Render dosya sisteminde
instance yaşamı boyunca kalır; GitHub'daki dosyayı kendiliğinden değiştirmez.
Kalıcı arşiv için güncellenen Excel'i GitHub'a yüklemek veya kalıcı disk
kullanmak gerekir.
