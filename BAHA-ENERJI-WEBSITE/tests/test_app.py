...
class SuiteHelpersTests(unittest.TestCase):
    def test_epias_client_is_reused_for_the_complete_web_session(self):
        token = APP.AUTH.create_session(
            "cache-test@baha.local",
            "TGT-cache-test",
        )
        try:
            session = APP.AUTH.get_session(token)
            first = APP._epias_client_for_session(token, session)
            second = APP._epias_client_for_session(token, session)

            self.assertIs(first, second)
            self.assertEqual(first.get_tgt(), "TGT-cache-test")
            self.assertEqual(first.username, "")
            self.assertEqual(first.password, "")

            APP._drop_epias_session_client(token)
            third = APP._epias_client_for_session(token, session)
    ...
    def test_system_direction_always_publishes_best_direction(self):
        low_confidence = APP._system_direction_decision(
            "deficit",
            44,
            {"deficit": 54, "surplus": 38, "balanced": 8},
        )
        narrow_margin = APP._system_direction_decision(
            "surplus",
            62,
            {"deficit": 35, "surplus": 39, "balanced": 26},
        )
        confident = APP._system_direction_decision(
            "surplus",
            68,
            {"deficit": 20, "surplus": 68, "balanced": 12},
        )

        self.assertEqual(low_confidence["category"], "deficit")
        self.assertFalse(low_confidence["abstained"])
        self.assertTrue(low_confidence["lowConfidence"])
        self.assertEqual(narrow_margin["category"], "surplus")
        self.assertFalse(narrow_margin["abstained"])
        self.assertTrue(narrow_margin["lowConfidence"])
        self.assertEqual(confident["category"], "surplus")
        self.assertFalse(confident["abstained"])
        self.assertFalse(confident["lowConfidence"])

    def test_system_direction_quality_contains_daily_trend_and_coverage(self):
        target_day = date(2026, 7, 29)
        records = [
            {
                "sampleDay": date(2026, 7, 27),
                "date": "2026-07-27",
                "hour": 0,
                "actual": "deficit",
                "correct": True,
        ...
        quality = APP._summarize_system_direction_quality(
            records,
            target_day,
        )

        self.assertEqual(len(quality["daily"]), 2)
        self.assertEqual(quality["windows"]["lastDay"]["date"], "2026-07-28")
        self.assertEqual(quality["windows"]["last7"]["coverage"], 66.7)
        self.assertEqual(quality["windows"]["last7"]["abstainedHours"], 1)
    ...
    def test_forecast_and_validation_share_locked_forecast_rows(self):
        original_enabled = APP.SYSTEM_DIRECTION_LEDGER_ENABLED
        original_path = APP.SYSTEM_DIRECTION_LEDGER_PATH
        ledger_path = (
            ROOT / "tests" / ".system-direction-shared-ledger-test.sqlite3"
        )
        try:
            ledger_path.unlink(missing_ok=True)
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = True
                ...
                "forecastRows": [
                    {
                        ...
                        "time": f"{hour:02}:00",
                        "category": (
                            "deficit" if hour < 12 else "surplus"
                        ),
                        "label": (
                            "Enerji Açığı"
                            if hour < 12
                        ...
                        "confidence": 70.0,
                        "probabilities": {
                            "deficit": 70 if hour < 12 else 20,
                            "surplus": 20 if hour < 12 else 70,
                            "balanced": 10,
                        },
                        "observed": False,
                    }
            ...
            stored = APP._system_direction_ledger_store(locked_payload)
            self.assertEqual(stored["rowCount"], 24)
            ...
            recalculated_payload = {
                **locked_payload,
                "forecastRows": [
                    ...
                    for row in locked_payload["forecastRows"]
                ],
                "summary": {},
                "schedule": {},
            }
            status = APP._system_direction_apply_ledger(
                recalculated_payload,
                APP._system_direction_ledger_read(date(2026, 7, 29)),
            )

            self.assertEqual(status["status"], "locked")
            self.assertEqual(
                recalculated_payload["forecastSource"],
                "locked_ledger",
            )
            self.assertEqual(
                recalculated_payload["forecastRows"][0]["category"],
                "deficit",
            )
            self.assertEqual(
                recalculated_payload["forecastRows"][12]["category"],
                "surplus",
            )
            self.assertTrue(
                all(
                    row["observed"] is False
                    for row in recalculated_payload["forecastRows"]
                )
            )
            self.assertIn(
                "01 ve 02",
                recalculated_payload["schedule"]["detail"],
            )
        finally:
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = original_enabled
            APP.SYSTEM_DIRECTION_LEDGER_PATH = original_path
    ...
    def test_partial_direction_ledger_is_completed_without_target_actuals(self):
        original_enabled = APP.SYSTEM_DIRECTION_LEDGER_ENABLED
        original_path = APP.SYSTEM_DIRECTION_LEDGER_PATH
        ledger_path = ROOT / "tests" / ".system-direction-hybrid-test.sqlite3"
        target_day = datetime.now(APP.URETIM.TR_TZ).date() - timedelta(days=2)
        ...
        try:
            ...
            partial = {
                ...
                "forecastMode": "intraday",
                "generatedAt": f"{target_day.isoformat()}T10:00:00Z",
                "calendarContext": {"label": "Normal iş günü"},
                "forecastRows": [
                    {
                        "hour": hour,
            ...
            stored = APP._system_direction_ledger_store(partial)
            self.assertEqual(stored["rowCount"], 14)
            ...
            self.assertEqual(forecast["forecastSource"], "hybrid_ledger")
            self.assertEqual(forecast["forecastLedger"]["status"], "hybrid")
            self.assertEqual(len(forecast["forecastRows"]), 24)
            self.assertEqual(forecast["summary"]["forecastHours"], 24)
            self.assertTrue(forecast["forecastRows"][0]["reconstructed"])
            self.assertEqual(
                forecast["forecastRows"][0]["category"],
                "deficit",
            )
            self.assertTrue(forecast["forecastRows"][10]["recorded"])
            self.assertEqual(
                forecast["forecastRows"][10]["category"],
                "balanced",
            )

            validation = APP._system_direction_validation(
                target_day.isoformat(),
                StubClient(),
                force_refresh=True,
            )
            self.assertEqual(
                validation["forecastSource"],
                "hybrid_ledger",
            )
            self.assertEqual(validation["summary"]["comparedHours"], 24)
            self.assertNotIn(
                "missing",
                {
    ...
    def test_send_content_ignores_client_abort(self):
        class ClosedSocket:
        ...
        calls = []
        handler._encoded_content = lambda content, _content_type: (content, {})
        ...
        APP.RequestHandler._send_content(
            handler,
            b"payload",
            "application/json; charset=utf-8",
        )

        self.assertIn(("status", APP.HTTPStatus.OK), calls)

    def test_background_refresh_defaults_use_safe_intervals(self):
        service = APP.BackgroundRefreshService()
        snapshot = service.snapshot()

        self.assertTrue(snapshot["enabled"])
        self.assertFalse(snapshot["running"])
        self.assertGreaterEqual(snapshot["taskIntervals"]["piyasa"], 60)
        self.assertGreaterEqual(snapshot["taskIntervals"]["ertesiPtf"], 120)
        self.assertGreaterEqual(snapshot["taskIntervals"]["tuketim"], 120)
        self.assertGreaterEqual(snapshot["taskIntervals"]["baraj"], 3600)
        self.assertGreaterEqual(snapshot["taskIntervals"]["uretim"], 900)
        self.assertEqual(snapshot["barajSchedule"]["startHour"], 8)
        self.assertEqual(snapshot["barajSchedule"]["endHour"], 19)
        timezone = APP.URETIM.TR_TZ
        self.assertIsNone(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 7, 59, tzinfo=timezone))
        )
        self.assertEqual(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 8, 0, tzinfo=timezone)),
            "2026-08-05T08",
        )
        self.assertEqual(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 19, 59, tzinfo=timezone)),
            "2026-08-05T19",
        )
        self.assertIsNone(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 20, 0, tzinfo=timezone))
        )
    ...
    def test_rewrite_paths_keeps_shared_suite_assets_at_the_site_root(self):
        ...
        chart_source = '"/piyasa-charts.js?v=19"'
        self.assertEqual(APP._rewrite_paths(chart_source, "/piyasa"), chart_source)
        self.assertEqual(APP._rewrite_paths('"/login"', "/piyasa"), '"/login"')
    ...
    def test_suite_navigation_marks_active_module(self):
        ...
        self.assertIn('href="/sistem-yonu-tahmini/"', navigation)
        self.assertIn('class="suite-nav-forecast"', navigation)
        self.assertIn("Sistem Yönü Tahmini", navigation)
        self.assertNotIn("suite-nav-short", navigation)
    ...
    def test_module_sidebar_uses_matching_sections(self):
        ...
        sistem = APP._module_sidebar("sistem")
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-summary"', baraj)
        self.assertIn(
            'href="/baraj/" data-suite-scroll="#baraj-regime-analysis"',
            baraj,
        )
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-map"', baraj)
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-compare"', baraj)
        self.assertIn("Karşılaştır", baraj)
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-list"', baraj)
        self.assertNotIn('href="#basin-risk"', baraj)
        self.assertIn('href="/uretim/" data-suite-scroll="#trendTitle"', uretim)
        self.assertIn('href="/uretim/" data-suite-scroll="#detailsTitle"', uretim)
        tuketim = APP._module_sidebar("tuketim")
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-summary"',
            tuketim,
        )
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-chart"',
            tuketim,
        )
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-table"',
            tuketim,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-timeline"',
            sistem,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-samples"',
            sistem,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-validation"',
            sistem,
        )
        self.assertIn("Doğrulama", sistem)
        self.assertIn("Sistem Yönü Tahmini", sistem)
        ...
        self.assertIn("EPİAŞ · EPİAŞ canlı", sistem)
        self.assertIn('class="suite-menu-close"', baraj)
        self.assertIn('class="suite-menu-close"', uretim)
        self.assertIn('class="suite-menu-close"', sistem)
        self.assertIn('id="sidebar-lock"', baraj)
        self.assertIn('id="sidebar-lock"', uretim)
        self.assertIn('id="sidebar-lock"', sistem)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', baraj)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', uretim)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', sistem)
        self.assertIn('aria-label="Yan menüyü sabitle"', baraj)
    ...
    def test_system_direction_forecast_uses_historical_direction_patterns(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                if not endpoint.endswith("/system-direction"):
                    raise AssertionError(endpoint)
                ...
                while current <= end:
                    for hour in range(24):
                        if hour < 7:
                            ...
                            direction = "Enerji Fazlası"
                        else:
                            direction = "Dengede"
                        items.append(
                            {
                                "date": f"{current.isoformat()}T{hour:02}:00:00+03:00",
                                "hour": hour,
                                "systemDirection": direction,
                            }
                        )
        ...
        target_day = datetime.now(APP.URETIM.TR_TZ).date() + timedelta(days=1)
        forecast = APP._system_direction_forecast(
            target_day.isoformat(),
            StubClient(),
            force_refresh=True,
        ...
        self.assertEqual(forecast["targetDate"], target_day.isoformat())
        self.assertEqual(len(forecast["forecastRows"]), 24)
        self.assertEqual(forecast["forecastRows"][0]["category"], "deficit")
        self.assertEqual(forecast["forecastRows"][9]["category"], "surplus")
        self.assertEqual(forecast["forecastRows"][23]["category"], "balanced")
        self.assertEqual(
            forecast["summary"]["predictedCounts"],
            {"deficit": 7, "surplus": 15, "balanced": 2},
        )
        self.assertGreater(forecast["summary"]["sampleCount"], 10)
        self.assertGreater(forecast["summary"]["averageConfidence"], 65)
        self.assertEqual(forecast["forecastMode"], "day_ahead")
        self.assertTrue(
            forecast["modelSummary"]["machineLearning"]["available"]
        )
        self.assertGreater(
            forecast["modelSummary"]["machineLearning"][
                "validationAccuracy"
            ],
            90,
        )
        self.assertGreater(
            forecast["modelSummary"]["averageWeights"]["ml"],
            0,
        )
        self.assertGreater(
            forecast["modelSummary"]["averageWeights"][
                "transition"
            ],
            0,
        )
        ...
        bucket_labels = [
            bucket["label"] for bucket in forecast["method"]["sourceBuckets"]
        ]
        self.assertIn("Geçmiş yıllar aynı ay/hafta/gün", bucket_labels)

    def test_system_direction_samples_include_same_month_weekday_in_past_years(self):
        target_day = date(2026, 7, 14)  # Temmuz ayının 2. Salı günü
        specs = APP._system_direction_sample_specs(
            target_day,
            target_day - timedelta(days=1),
        )

        self.assertEqual(
            APP._same_month_weekday_in_year(target_day, 2025),
            date(2025, 7, 8),
        )
        self.assertEqual(
            APP._same_month_weekday_in_year(target_day, 2024),
            date(2024, 7, 9),
        )
        self.assertIn(
            "calendar_yearly",
        ...
        self.assertEqual(
            APP._same_month_weekday_in_year(date(2026, 3, 31), 2025),
            date(2025, 3, 25),
        )
        self.assertIn("recent", specs[target_day - timedelta(days=28)])
        self.assertIn("weekly", specs[target_day - timedelta(days=12 * 7)])
        self.assertIn(
            "seasonal_yearly",
            specs[date(2025, 7, 8) - timedelta(days=7)],
        )

    def test_system_direction_schedule_switches_at_18_turkey_time(self):
        before = APP._system_direction_schedule(
            datetime(2026, 7, 29, 17, 59, tzinfo=APP.URETIM.TR_TZ)
        )
        after = APP._system_direction_schedule(
            datetime(2026, 7, 29, 18, 0, tzinfo=APP.URETIM.TR_TZ)
        )

        self.assertEqual(before["phase"], "today")
        self.assertEqual(before["targetDate"], "2026-07-29")
        self.assertEqual(before["validationDate"], "2026-07-29")
        self.assertEqual(after["phase"], "tomorrow")
        self.assertEqual(after["targetDate"], "2026-07-30")
        self.assertEqual(after["validationDate"], "2026-07-29")

    def test_direction_features_keep_target_actual_out_of_day_ahead_model(self):
        target_day = date(2026, 7, 29)
        history = {}
        for offset in range(1, 36):
            sample_day = target_day - timedelta(days=offset)
            history[sample_day] = {
                hour: {
                    "hour": hour,
                    "category": (
        ...
        first_index = {
            **history,
            target_day: {
                hour: {"hour": hour, "category": "surplus"}
                for hour in range(24)
            },
        }
        second_index = {
            **history,
            target_day: {
                hour: {"hour": hour, "category": "deficit"}
                for hour in range(24)
            },
        }

        first = APP._direction_day_features(target_day, 12, first_index)
        second = APP._direction_day_features(target_day, 12, second_index)
        intraday = APP._direction_day_features(
            target_day,
            12,
            first_index,
            observed_by_hour=first_index[target_day],
            observed_cutoff=9,
        )
    ...
    def test_softmax_direction_model_learns_repeating_classes(self):
        ...
        for index in range(180):
            ...
            features.append(
                [
                    1.0 if category_index == 0 else 0.0,
                    1.0 if category_index == 1 else 0.0,
                    1.0 if category_index == 2 else 0.0,
                ]
            )
            labels.append(APP.DIRECTION_CATEGORIES[category_index])

        model = APP._softmax_direction_model(features, labels)
        probabilities = APP._softmax_direction_probabilities(
            model,
            [0.0, 1.0, 0.0],
        )

        self.assertTrue(model["available"])
        self.assertGreater(model["validationAccuracy"], 95)
        self.assertEqual(
            max(probabilities, key=probabilities.get),
            "surplus",
        )

    def test_direction_ml_model_reuses_training_cache_until_history_changes(self):
        target_day = date(2026, 7, 29)
        history = {
            target_day - timedelta(days=offset): {
                hour: {
                    "hour": hour,
                    "category": "deficit" if hour < 8 else "surplus",
                }
                for hour in range(24)
            }
        ...
        first = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )
        second = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )

        self.assertFalse(first["trainingCacheHit"])
        self.assertTrue(second["trainingCacheHit"])
        self.assertEqual(first["sampleCount"], second["sampleCount"])

        changed_day = target_day - timedelta(days=1)
        history[changed_day][0]["category"] = "balanced"
        changed = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )
        self.assertFalse(changed["trainingCacheHit"])

    def test_system_direction_quality_has_windows_categories_and_hours(self):
        target_day = date(2026, 7, 29)
        records = []
        for days_ago in range(1, 11):
            sample_day = target_day - timedelta(days=days_ago)
            for hour in range(24):
                actual = "deficit" if hour < 8 else "surplus"
                records.append(
                    {
                        ...
                        "hour": hour,
                        "actual": actual,
                        "predicted": actual if hour != 3 else "balanced",
                        "confidence": 80,
                        "correct": hour != 3,
                    }
                )
        ...
        self.assertEqual(quality["windows"]["last7"]["dayCount"], 7)
        self.assertEqual(quality["windows"]["last30"]["dayCount"], 10)
        self.assertEqual(len(quality["hours"]), 24)
        self.assertEqual(quality["hours"][3]["accuracy"], 0)
        self.assertEqual(quality["hours"][4]["accuracy"], 100)
        self.assertEqual(
            quality["categories"]["surplus"]["accuracy"],
            100,
        )

    def test_transition_model_uses_regime_duration(self):
        target_day = date(2026, 7, 29)
        history = {}
        for offset in range(1, 43):
            ...
            history[sample_day] = {
                hour: {
                    "hour": hour,
                    "category": (
                        "deficit" if hour < 9 else "surplus"
                    ),
                }
                for hour in range(24)
            }

        model = APP._system_direction_transition_model(
            target_day,
            history,
        )
        probabilities = APP._transition_probabilities(
            model,
            6,
            "deficit",
            6,
        ...
        self.assertGreater(model["accuracy"], 90)
        self.assertGreater(
            probabilities["deficit"],
            probabilities["surplus"],
        )

    def test_operational_signal_combines_balancing_price_and_demand_error(self):
        rows = [
            {
                "hour": 8,
                "yal": 180,
        ...
        probabilities = (
            APP._system_direction_operational_probabilities(
                rows,
                8,
                9,
    ...
    def test_ensemble_gives_better_validated_model_more_weight(self):
        weights = APP._system_direction_ensemble_weights(
            52,
            81,
            63,
        )

        self.assertGreater(weights["ml"], weights["history"])
        self.assertGreater(weights["ml"], weights["transition"])
        self.assertAlmostEqual(sum(weights.values()), 1.0, places=3)

    def test_system_direction_context_combines_demand_supply_and_weather(self):
        scores = {
            hour: {"deficit": 10.0, "surplus": 10.0, "balanced": 10.0}
            for hour in range(24)
        }
        context = {
            "weather": {
            ...
            "kgup": {
                "rows": [
                    {"hour": 0, "renewableShare": 50},
                    {"hour": 1, "renewableShare": 10},
                ]
            },
            "consumption": {
                "rows": [
        ...
        adjusted, details = APP._apply_system_direction_context(scores, context)

        self.assertGreater(adjusted[1]["deficit"], adjusted[1]["surplus"])
        self.assertGreater(details[1]["balancePressure"], 0.2)
        labels = [signal["label"] for signal in details[1]["signals"]]
        self.assertIn("Yüksek tüketim", labels)
        self.assertIn("Düşük yenilenebilir planı", labels)
        self.assertIn("Sıcaklık kaynaklı talep", labels)

    def test_system_direction_calibration_corrects_repeated_early_transition(self):
        scores = {
            hour: {"deficit": 4.0, "surplus": 3.0, "balanced": 1.0}
            for hour in range(24)
        }
        scores[3] = {"deficit": 4.0, "surplus": 6.0, "balanced": 1.0}
        calibration = {
            "confusion": {
                3: {
                    "surplus": {
            ...
            "transitionHolds": {
                3: {"deficit": 5.0, "surplus": 0.0, "balanced": 0.0}
            },
        }

        adjusted, details = APP._apply_system_direction_calibration(
            scores,
            calibration,
        )

        self.assertGreater(adjusted[3]["deficit"], adjusted[3]["surplus"])
    ...
    def test_system_direction_miss_reason_detects_delayed_regime_change(self):
        forecast = {
            2: {"hour": 2, "category": "deficit"},
            3: {"hour": 3, "category": "surplus"},
            4: {"hour": 4, "category": "surplus"},
        }
        actual = {
            2: {"hour": 2, "category": "deficit"},
            3: {"hour": 3, "category": "deficit"},
            4: {"hour": 4, "category": "deficit"},
        }

        first_miss = APP._system_direction_miss_reason(3, forecast, actual)
        second_miss = APP._system_direction_miss_reason(4, forecast, actual)

        self.assertEqual(first_miss["code"], "early_transition")
        self.assertIn("1 saat daha sürdü", first_miss["text"])
        self.assertEqual(second_miss["code"], "early_transition")
        self.assertIn("2 saat daha sürdü", second_miss["text"])

    def test_system_direction_kgup_reads_hourly_generation_mix(self):
        class StubClient:
            def _post_json(self, endpoint, body):
        ...
        client = StubClient()
        result = APP._system_direction_kgup(date(2026, 7, 30), client)

        self.assertEqual(client.endpoint, "/v1/generation/data/dpp")
        self.assertEqual(client.body["region"], "TR1")
        self.assertEqual(result["availableHours"], 1)
        self.assertEqual(result["rows"][0]["renewable"], 11_500)
        self.assertEqual(result["rows"][0]["renewableShare"], 23.0)

    def test_system_direction_weather_aggregates_hourly_forecast(self):
        request_options = {}
        ...
        class FakeResponse:
            ...
            def read(self):
                hourly = {
                    "time": [
                        f"2026-07-30T{hour:02}:00"
                        for hour in range(24)
                    ]
                }
                for field in APP.SYSTEM_DIRECTION_WEATHER_FIELDS:
                    hourly[field] = [20 + hour for hour in range(24)]
        ...
        try:
            with APP.SYSTEM_DIRECTION_WEATHER_CACHE_LOCK:
                APP.SYSTEM_DIRECTION_WEATHER_CACHE.clear()
            result = APP._system_direction_weather(
                date(2026, 7, 30),
                force_refresh=True,
            )
        finally:
        ...
        self.assertEqual(result["availableHours"], 24)
        self.assertEqual(result["pointCount"], 1)
        self.assertEqual(result["rows"][0]["temperature_2m"], 20)
        self.assertEqual(result["rows"][23]["wind_speed_80m"], 43)
        self.assertEqual(request_options["timeout"], 8)
        self.assertEqual(request_options["context"].verify_mode, APP.ssl.CERT_REQUIRED)
        self.assertTrue(request_options["context"].check_hostname)

    def test_system_direction_validation_compares_forecast_with_actual_hours(self):
        validation_day = datetime.now(APP.URETIM.TR_TZ).date()

        class StubClient:
            actual_day = validation_day
                ...
                while current <= end:
                    for hour in range(24):
                        if hour < 7:
                            ...
                            direction = "Dengede"
                        if current == self.actual_day and hour == 9:
                            direction = "Enerji Açığı"
                        items.append(
                            {
                                "date": f"{current.isoformat()}T{hour:02}:00:00+03:00",
        ...
        self.assertEqual(validation["date"], validation_day.isoformat())
        self.assertEqual(len(validation["rows"]), 24)
        self.assertEqual(validation["summary"]["publishedHours"], 24)
        self.assertEqual(validation["summary"]["comparedHours"], 24)
        self.assertEqual(validation["summary"]["correctHours"], 23)
        self.assertEqual(validation["summary"]["wrongHours"], 1)
        self.assertEqual(validation["summary"]["accuracy"], 95.8)
        self.assertEqual(validation["summary"]["status"], "ready")
        self.assertFalse(validation["rows"][9]["match"])
        self.assertEqual(validation["rows"][9]["forecastCategory"], "surplus")
    ...
    def test_basin_regime_analysis_includes_weekly_and_monthly_drop(self):
        ...
        daily = analysis["periodComparisons"]["daily"]
        ...
        self.assertEqual(daily["actualDays"], 1)
        self.assertAlmostEqual(daily["drop"], 1)
    ...
    def test_basin_regime_daily_comparison_requires_previous_calendar_day(self):
        analysis = APP._basin_regime_analysis(
            [
                {"date": "2026-07-01", "average": 70},
                {"date": "2026-07-03", "average": 68},
            ]
        )

        daily = analysis["periodComparisons"]["daily"]
        self.assertFalse(daily["available"])
        self.assertIsNone(daily["baselineDate"])
        self.assertIsNone(daily["drop"])
    ...
    def test_basin_risk_combines_fullness_decline_and_critical_horizon(self):
        ...
        self.assertIn("günde ortalama", risk["reason"])
        self.assertIn("%30 seviyesine", risk["reason"])
    ...
    def test_market_today_incomplete_smf_uses_short_refresh_policy(self):
        selected_date = APP.datetime.now(APP.URETIM.TR_TZ).date().isoformat()
        APP.MARKET_CACHE.pop(selected_date, None)
        ...
        class StubClient:
            def _post_json(self, endpoint, body):
                ...
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [
                            {"hour": 0, "systemMarginalPrice": 0},
                            {"hour": 1, "systemMarginalPrice": 1_100},
                        ]
                    }
                return {"items": []}

        before = APP.time.time()
        dashboard = APP._market_dashboard(selected_date, StubClient())
        self.assertEqual(dashboard["freshness"]["smfPublishedHours"], 2)
        self.assertTrue(dashboard["freshness"]["smfIncomplete"])
        self.assertEqual(dashboard["freshness"]["clientCacheMs"], 15_000)
        self.assertEqual(dashboard["freshness"]["nextRefreshMs"], 45_000)
        self.assertEqual(dashboard["summary"]["ptfSmfCommonHours"], 2)
        self.assertEqual(dashboard["summary"]["smfAverage"], 550)
        self.assertLessEqual(
            APP.MARKET_CACHE[selected_date]["expires"] - before,
            25,
        )

    def test_market_historical_zero_smf_is_a_published_value(self):
            ...
            def _post_json(self, endpoint, body):
                if endpoint.endswith("/mcp"):
                    return {
                        "items": [
                            {
                                "hour": "2026-07-11T12:00:00+03:00",
                                "price": 2_000,
                            }
                        ]
                    }
        ...
        self.assertEqual(dashboard["rows"][0]["hour"], 12)
        self.assertEqual(dashboard["rows"][0]["smf"], 0)
        self.assertTrue(dashboard["rows"][0]["smfPublished"])
        self.assertEqual(dashboard["freshness"]["smfPublishedHours"], 1)
        self.assertEqual(dashboard["summary"]["ptfSmfCommonHours"], 1)
        self.assertEqual(dashboard["summary"]["smfAverage"], 0)
    ...
    def test_consumption_forecast_builds_24_hour_weighted_profile(self):
        ...
        APP.CONSUMPTION_FORECAST_LOCKED_ROWS.clear()
        client = StubClient()
    ...
    def test_consumption_forecast_can_target_selected_day_and_marks_locks(self):
        today = datetime.now(APP.URETIM.TR_TZ).date()
        ...
        APP.CONSUMPTION_FORECAST_LOCKED_ROWS.clear()
        client = StubClient()
        payload = APP._consumption_forecast(
            today.isoformat(),
            client,
        ...
        self.assertEqual(
            client.calls[0][1]["endDate"],
            f"{training_end.isoformat()}T00:00:00+03:00",
        )
        self.assertEqual(payload["summary"]["forecastHours"], 24)
        self.assertIn("lockedHours", payload["summary"])
        self.assertIn("finalUpdateAt", payload["rows"][0])
        self.assertIn("hourStartsAt", payload["rows"][0])
    ...
    def test_baraj_archive_appends_only_a_new_complete_day(self):
        import openpyxl

        target = ROOT / "tests" / f".baraj-archive-{os.getpid()}.xlsx"
        workbook = openpyxl.Workbook()
        raw = workbook.active
        raw.title = "Aktif Doluluk"
        raw.append(["Tarih", "Havza", "Baraj", "Aktif Doluluk (%)"])
        raw.append([datetime(2026, 8, 4), "Havza A", "BARAJ A", 60])
        raw.append([datetime(2026, 8, 4), "Havza B", "BARAJ B", 40])
        pivot = workbook.create_sheet("Pivot")
        pivot.cell(3, 1).value = "Ortalama Aktif Doluluk (%)"
        pivot.cell(3, 2).value = "Tarih"
        pivot.cell(4, 1).value = "Barajlar"
        pivot.cell(4, 2).value = datetime(2026, 8, 4)
        pivot.cell(4, 3).value = "Genel Ortalama"
        pivot.cell(5, 1).value = "BARAJ A"
        pivot.cell(5, 2).value = 60
        pivot.cell(5, 3).value = 60
        pivot.cell(6, 1).value = "BARAJ B"
        pivot.cell(6, 2).value = 40
        pivot.cell(6, 3).value = 40
        pivot.cell(7, 1).value = "Genel Ortalama"
        pivot.cell(7, 2).value = 50
        pivot.cell(7, 3).value = 50
        ...
        rows = [
            {
                ...
                "basin": "Havza A",
                "dam": "BARAJ A",
                "activeFullnessAmount": 999,
            },
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza A",
                "dam": "BARAJ A",
                "activeFullnessAmount": 59,
            },
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza B",
                "dam": "BARAJ B",
                "activeFullnessAmount": 41,
            },
        ]
        try:
            first = APP.append_active_fullness_days(
                target,
                rows,
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            second = APP.append_active_fullness_days(
                target,
                rows,
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            archive = APP._load_baraj_archive(target)
            self.assertTrue(first.updated)
            self.assertEqual(first.added_dates, ("2026-08-05",))
            self.assertEqual(first.added_rows, 2)
            self.assertFalse(second.updated)
            self.assertEqual(archive["availableDates"], ["2026-08-04", "2026-08-05"])
            self.assertEqual(len(archive["byDate"]["2026-08-05"]), 2)
            old_baraj = next(
                item
                for item in archive["byDate"]["2026-08-04"]
                if item["dam"] == "BARAJ A"
            )
            self.assertEqual(old_baraj["activeFullnessAmount"], 60)
        finally:
    ...
    def test_executive_report_builds_branded_html_and_five_sheet_xlsx(self):
        report = {
            ...
            "comparisons": [
                ...
                },
                {
                    ...
                    "damAverage": 70,
                    "uevmTotal": 930,
                    "consumptionAverage": 41000,
                    "delta": {
                        "ptfAverage": 200,
                        "smfAverage": 50,
                        "damAverage": 2.5,
                        "uevmTotal": 70,
                        "consumptionAverage": 2000,
                    },
                },
            ],
            "damSummary": {
        ...
        self.assertIn("TAKVİM KIYASI", html)
        self.assertIn("Geçen yıl aynı hafta-gün", html)
        self.assertIn("+200,00 fark", html)
        ...
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            ...
            self.assertIn('name="Takvim Kıyası"', workbook_xml)
    ...
    def test_executive_dashboard_combines_all_modules_for_one_date(self):
        ...
        calls = []
        def fake_market_dashboard(selected, client):
            calls.append(("market", selected))
            baseline = 100 if selected == "2026-07-20" else 90
            return {
                "summary": {
                    "ptfAverageByCurrency": {"TRY": baseline},
                    "smfAverage": baseline - 5,
                },
                "rows": [],
            }

        APP._market_dashboard = fake_market_dashboard
        ...
        def fake_production_dashboard(date_range, client):
            selected = date_range.start.isoformat()
            calls.append(("production", selected, date_range.end.isoformat()))
            return {
                "summary": {
                    "uevmTotal": 1000 if selected == "2026-07-20" else 900,
                },
                "series": [],
            }

        APP.URETIM_SERVICE.dashboard = fake_production_dashboard
        APP._consumption_dashboard = lambda selected, client: calls.append(("consumption", selected)) or {"summary": {"average": 40000 if selected == "2026-07-20" else 39000}, "rows": []}
        try:
        ...
        self.assertEqual(report["comparisons"][0]["label"], "Seçili gün")
        self.assertEqual(report["comparisons"][3]["key"], "lastYearSameWeekday")
        self.assertEqual(report["comparisons"][3]["date"], "2025-07-21")
        self.assertEqual(report["comparisons"][1]["delta"]["ptfAverage"], 10)
        self.assertIn(("production", "2026-07-20", "2026-07-20"), calls)

    def test_command_center_uses_latest_shared_market_payload(self):
        originals = (APP._executive_dashboard, APP._market_dashboard)
        dashboard_options = []
        APP._executive_dashboard = lambda selected, client, **kwargs: dashboard_options.append(kwargs) or {
            "date": selected,
            "generatedAt": "2026-07-22T09:00:00+03:00",
            "modules": {"market": {"rows": [{"time": "09:00", "smf": None}]}},
            "errors": {"market": "Eski piyasa kopyası"},
            "availableModules": [],
            "cached": True,
        }
        APP._market_dashboard = lambda selected, client: {
            "updatedAt": "2026-07-22T06:01:00Z",
            "rows": [{"time": "09:00", "smf": 2450, "smfPublished": True}],
            "freshness": {"smfIncomplete": True, "nextRefreshMs": 45000},
            "cached": True,
        }
        try:
            report = APP._command_center_dashboard("2026-07-22", object())
        finally:
            APP._executive_dashboard, APP._market_dashboard = originals
        self.assertEqual(report["modules"]["market"]["rows"][0]["smf"], 2450)
        self.assertTrue(report["modules"]["market"]["rows"][0]["smfPublished"])
        self.assertNotIn("market", report["errors"])
        self.assertIn("market", report["availableModules"])
        self.assertEqual(dashboard_options, [{"include_comparisons": False}])
...
class SuiteHttpTests(unittest.TestCase):
    ...
    def test_epias_guard_api_reports_safe_limits(self):
        status, content, _ = self.get("/api/epias-limits")
        payload = json.loads(content)

        self.assertEqual(status, 200)
        self.assertEqual(payload["configured"]["tgt"]["perMinute"], 10)
        self.assertEqual(payload["configured"]["api"]["perMinute"], 24)
        self.assertEqual(
            payload["configured"]["usernameFailedAttempts"]["maxAttempts"],
            3,
        )
        self.assertTrue(payload["configured"]["singleFlight"]["enabled"])
        self.assertIn("background", payload)
        self.assertIn("taskIntervals", payload["background"])
        self.assertGreaterEqual(payload["background"]["taskIntervals"]["uretim"], 900)

    def test_epias_guard_page_is_available_to_authenticated_users(self):
        status, content, _ = self.get("/epias-koruma")
        html = content.decode("utf-8")

        self.assertEqual(status, 200)
        self.assertIn("Limit ve koruma paneli", html)
        self.assertIn("Arka plan veri toplama", html)
        self.assertIn("/epias-guard.js", html)
        self.assertIn("EPİAŞ koruma", html)

    def test_large_text_assets_are_gzip_compressed_when_requested(self):
        ...
        try:
            connection.request(
                "GET",
                "/baraj/turkiye-havzalari.geojson",
                headers={
            ...
            self.assertLess(len(compressed), len(decompressed))
            self.assertIn(b'"FeatureCollection"', decompressed)
        finally:
            connection.close()

    def test_versioned_assets_use_long_browser_cache(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
    ...
    def test_unchanged_static_asset_returns_not_modified(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        headers = {"Cookie": f"{APP.AUTH.cookie_name}={self.token}"}
        try:
            connection.request("GET", "/module-suite.css?v=54", headers=headers)
    ...
    def test_login_endpoint_rate_limits_repeated_failures_by_username(self):
        original = APP.LOGIN_USERNAME_LIMITER
        limiter = APP.LoginRateLimiter(
            max_attempts=1,
            window_seconds=10,
            block_seconds=60,
        )
        limiter.record_failure("username:test@example.com")
        APP.LOGIN_USERNAME_LIMITER = limiter
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
            ...
            APP.LOGIN_USERNAME_LIMITER = original
            connection.close()
    ...
    def test_login_is_a_clean_shared_portal_page(self):
        ...
        self.assertIn("10 saniyede 3 hatalı", html)
        ...
        self.assertIn('id="bahaSuiteLoading"', html)
        self.assertIn("Oturum kontrol ediliyor", html)
        self.assertIn('src="/suite-loading.js?v=3"', html)
        self.assertIn("baha-suite-loading-failsafe", html)
        self.assertIn("Sunucu uyandırılıyor", html)
        self.assertIn("Panel yükleniyor", html)
        self.assertIn("Oturum kontrol ediliyor", html)
        ...
        self.assertEqual(status, 200)
        self.assertIn("baha-enerji-shell-v19", worker)
        self.assertIn("NAVIGATION_TIMEOUT_MS", worker)
        self.assertIn('const LOADING_PATH = "/panel-hazirlaniyor"', worker)
        self.assertIn('const READY_HEADER = "X-Baha-Panel"', worker)
        self.assertIn("isBahaPanelResponse", worker)
        self.assertIn("if (!useLoadingFallback)", worker)
        self.assertIn("/panel-hazirlaniyor", worker)
        self.assertIn('url.searchParams.has("baha_ready")', worker)
        self.assertIn('url.pathname === "/health"', worker)
        self.assertNotIn("baha-uretim-logo.svg", worker)

        status, content, _ = self.get("/piyasa/")
        piyasa_html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertNotIn('id="bahaSuiteLoading"', piyasa_html)
        self.assertNotIn("Panel hazırlanıyor...", piyasa_html)
        self.assertNotIn("/suite-loading.js", piyasa_html)

        status, content, loading_headers = self.get("/panel-hazirlaniyor")
        loading_html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertEqual(loading_headers.get("X-Baha-Panel"), "ready")
        self.assertIn("Sunucu uyandırılıyor", loading_html)
        self.assertIn('src="/panel-loading.js?v=4"', loading_html)
        self.assertIn('data-loading-step="0"', loading_html)
        self.assertIn("Sunucu uyandırılıyor", loading_html)

        status, content, _ = self.get("/panel-loading.js")
        loading_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("AbortController", loading_script)
        self.assertIn('payload?.status === "ok"', loading_script)
        self.assertIn('target.searchParams.set("baha_ready"', loading_script)
        self.assertIn("targetResponse.headers.get(\"X-Baha-Panel\")", loading_script)
        self.assertIn("window.location.replace(target.href)", loading_script)

        status, content, _ = self.get("/suite-loading.js")
        suite_loading_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("maxVisibleMs = 12000", suite_loading_script)
        self.assertIn("completeSessionCheck", suite_loading_script)
        self.assertIn('"DOMContentLoaded", finishInitialLoad', suite_loading_script)
        self.assertIn('"pageshow"', suite_loading_script)

        status, content, _ = self.get("/login.js")
        login_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("completeSessionCheck", login_script)
        self.assertIn('cache: "no-store"', login_script)
        ...
        self.assertIn("Dar masa", login_css)
        self.assertIn("(min-width: 521px) and (max-width: 900px)", login_css)
    ...
    def test_piyasa_page_paths_are_mounted_under_piyasa(self):
        ...
        self.assertIn('src="/piyasa/app.js?v=60"', html)
        ...
        self.assertIn('href="/piyasa/styles.css?v=37"', html)
        self.assertIn('href="/piyasa/" aria-current="page"', html)
        self.assertIn('src="/piyasa-charts.js?v=19"', html)
        self.assertIn("data-chart-engine", html)
        self.assertIn('href="/piyasa-suite.css?v=34"', html)
        self.assertIn('src="/theme-sync.js?v=3"', html)
        self.assertIn(
        ...
        self.assertIn('class="sidebar-lock-button"', html)
        self.assertIn('aria-label="Yan menüyü sabitle"', html)
        ...
        self.assertIn('id="smf-missing-chart-key"', html)
        self.assertIn("SMF yayınlanmadı", html)
        self.assertIn('data-chart-mode="full"', html)
        self.assertIn('data-chart-mode="simple"', html)
        self.assertIn('data-chart-mode="ptf"', html)
        self.assertIn('aria-label="Grafik görünümü"', html)
    ...
    def test_piyasa_never_shows_its_old_login_screen(self):
        ...
        self.assertNotIn("SMF aynı EPİAŞ gösterge kuruyla", script)
        self.assertIn("baha-market-currency", script)
        self.assertIn("baha-market-v6:", script)
        self.assertIn("smfPublished", script)
        self.assertIn("epias-ptf-direct", script)
        self.assertIn("supportsCurrencyPayload", script)
        self.assertIn("marketRefreshTimer", script)
        self.assertIn("scheduleMarketDataRefresh", script)
        self.assertIn("hasPendingSmf", script)
        self.assertIn("marketCacheTtl", script)
        self.assertIn("smfIncomplete", script)
        self.assertIn("clientCacheMs", script)
        ...
        self.assertIn("isSmfUnpublished", script)
        self.assertIn("unpublishedSmfValue", script)
        self.assertIn("publishedSmfValue", script)
        self.assertIn("lastPublishedSmfIndex", script)
        self.assertIn("isTrailingUnpublishedSmf", script)
        self.assertIn("firstTrailingUnpublishedSmfIndex", script)
        self.assertIn("smfBridgeValue", script)
        self.assertIn("index>last", script)
        self.assertIn("SMF geçiş", script)
        self.assertIn("SMF yayınlanmadı", script)
        self.assertIn("hideMarkers:true", script)
        self.assertIn("hideNullInTooltip:true", script)
        self.assertIn("hideInTooltip:true", script)
        self.assertIn("hideInLegend:true", script)
        self.assertNotIn("markerOnly:true", script)
        self.assertIn("colors.push('#8a67e8')", script)
        self.assertIn("colors.push('#d5573b')", script)
        self.assertIn("#d5573b", script)
        self.assertIn("{name:'PTF',data:currentRows.map(r=>priceValue(r,'ptf')),fill:false}", script)
        self.assertIn("{name:'SMF',data:currentRows.map(publishedSmfValue),fill:false}", script)
        self.assertIn("chart:{type:'line',height:320", script)
        self.assertIn("fill:{opacity:0}", script)
        self.assertIn("baha-sidebar-collapsed", script)
        self.assertIn("baha-sidebar-pinned", script)
        self.assertIn("setSidebarPinned", script)
        self.assertIn("suite-sidebar-pinned", script)
        self.assertIn("window.addEventListener('storage'", script)
        self.assertIn("desktopSidebar", script)
        self.assertIn("desktopLock", script)
        self.assertIn("canPinSidebar", script)
        self.assertIn("hover:hover", script)
        self.assertIn("pointer:fine", script)
        self.assertIn("setDesktopSidebarHover", script)
        self.assertIn("suite-sidebar-hovered", script)
        self.assertIn("suite-sidebar-dismissed", script)
        ...
        self.assertIn("ensureChartEngine", script)
        self.assertIn("loadChartEngineScript", script)
        self.assertIn("'/piyasa-charts.js?v=19'", script)
        self.assertIn("'piyasa-charts.js?v=19'", script)
        self.assertNotIn("/piyasa/piyasa/piyasa-charts.js", script)
        self.assertIn("renderMarketCharts", script)
        ...
        self.assertIn("syncMarketUrl", script)
        self.assertIn("applyInitialMarketUrlState", script)
        self.assertIn("compareDate", script)
        self.assertIn("priceChartMode", script)
        self.assertIn("baha-market-chart-mode", script)
        self.assertIn("showComparison", script)
        self.assertIn("priceChartMode==='full'", script)
        self.assertIn("priceChartMode!=='ptf'", script)

        status, content, headers = self.get("/piyasa/styles.css")
        styles = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".chart-key .smf-missing", styles)
        self.assertIn(".chart-key .smf-missing{color:inherit;font-weight:inherit}", styles)
        self.assertIn(".chart-key .smf-missing:before{background:#d5573b;box-shadow:none}", styles)
        self.assertNotIn("html[data-theme=dark] .chart-key .smf-missing{color:#fff}", styles)
        self.assertIn(".chart-mode", styles)
        self.assertIn(".sidebar-lock-button", styles)
        self.assertIn("@media(min-width:1025px) and (hover:hover) and (pointer:fine)", styles)
        self.assertIn("#d5573b", styles)
        self.assertEqual(headers.get_content_type(), "text/css")
    ...
    def test_piyasa_local_chart_and_styles_are_served(self):
        ...
        self.assertIn("window.__bahaEnergyCharts", chart_script)
        self.assertIn(
            'if (value === null || value === undefined || value === "") return null',
            chart_script,
        )
        self.assertIn("updateOptions(options)", chart_script)
        self.assertIn("this.renderedHeight", chart_script)
        ...
        self.assertIn("const center = xFor(categoryIndex)", chart_script)
        self.assertNotIn("categoryIndex + 0.5", chart_script)
        self.assertIn("strokeOptions.dashArray", chart_script)
        self.assertIn("item.fill !== false", chart_script)
        self.assertIn("item.markerOnly", chart_script)
        self.assertIn("item.markerRadius", chart_script)
        self.assertIn("item.markerFill", chart_script)
        self.assertIn("item.hideMarkers", chart_script)
        self.assertIn("item.hideInLegend", chart_script)
        self.assertIn("item.hideInTooltip", chart_script)
        self.assertIn("item.hideNullInTooltip", chart_script)
        self.assertIn("item.tooltipFormatter", chart_script)
        self.assertIn("const segments = []", chart_script)
        self.assertIn("currentSegment", chart_script)
        self.assertIn("segments.forEach", chart_script)
    ...
    def test_all_panels_include_shared_chart_fullscreen_controls(self):
        for path in ("/piyasa/", "/baraj/", "/uretim/", "/tuketim/"):
            with self.subTest(path=path):
                ...
                html = content.decode("utf-8")
                self.assertEqual(status, 200)
                self.assertIn('href="/chart-fullscreen.css?v=5"', html)
                self.assertIn('src="/chart-fullscreen.js?v=5"', html)
        ...
        self.assertIn("#consumptionChart", script)
        self.assertNotIn("#consumptionForecastChart", script)
        ...
        self.assertIn("forceApexDomHeight", script)
        self.assertIn("scheduleApexResize", script)
        self.assertIn("window.__bahaEnergyCharts", script)
        ...
        self.assertIn(".energy-chart > svg", css)
        self.assertIn(".energy-chart-legend", css)
        self.assertIn(
            "body.suite-chart-fullscreen-open.baha-suite-uretim .tooltip",
            css,
        )
        self.assertIn("z-index: 200020", css)
        ...
        self.assertIn(".baha-suite-piyasa .sidebar-lock-button", css)
        self.assertIn(
            "@media (min-width: 1025px) and (hover: hover) and (pointer: fine)",
            css,
        )
        self.assertIn(".baha-suite-piyasa.suite-sidebar-pinned .sidebar-lock-button", css)
        ...
        self.assertIn("suite-sidebar-dismissed", css)
    ...
    def test_baraj_inline_api_paths_are_mounted_under_baraj(self):
        ...
        self.assertNotIn("@tabler/core", html)
        ...
        self.assertIn('id="baraj-regime-analysis"', html)
        self.assertIn('id="basin-risk"', html)
        self.assertIn('id="basinRiskBody"', html)
        self.assertIn("Havzaların takip önceliği", html)
        self.assertIn("Neden bu sırada?", html)
        self.assertIn("Bu sıralama nasıl hesaplanıyor?", html)
        self.assertIn("resmî bir risk sınıflandırması değil", html)
        self.assertIn("baraj-risk-reason", html)
        self.assertIn('class="baraj-risk-summary"', html)
        self.assertIn("baraj-risk-change", html)
        ...
        self.assertIn("Dün / 7 gün / 30 gün", html)
        self.assertIn("periodComparisons.daily", html)
        ...
        self.assertIn("baraj-map-legend", html)
        self.assertIn("Harita doluluk renkleri", html)
        self.assertIn("fullness-low", html)
        self.assertIn("fullness-mid", html)
        self.assertIn("fullness-high", html)
        self.assertIn("basinFullnessColor", html)
        self.assertIn("path.style.setProperty('--basin-fill'", html)
        self.assertIn("fullness-known", html)
        ...
        self.assertIn('XLSX İNDİR</button>', html)
        self.assertNotIn('data-download-menu="baraj-list"', html)
        self.assertIn('id="barajDateSelect"', html)
        self.assertIn("initialBarajUrlState", html)
        self.assertIn("barajUrlState", html)
        self.assertIn("syncBarajUrlState", html)
        self.assertIn("applyInitialBarajUrlState", html)
        self.assertIn("BARAJ_CLIENT_CACHE_PREFIX", html)
        self.assertIn("readBarajClientCache", html)
        self.assertIn("load(true); loadBasinHistory()", html)
        self.assertIn("params.set('basin', basin)", html)
        self.assertIn("params.set('sort', sort)", html)
        ...
        self.assertIn("chart.clientHeight || 390", html)
        self.assertIn("let yMin = Math.max(0", html)
        self.assertIn("pointsByDate", html)
        self.assertIn("sort((first, second) => first.timestamp - second.timestamp)", html)
        ...
        self.assertIn('href="/module-suite.css?v=54"', html)
        self.assertIn('src="/module-suite.js?v=14"', html)
        self.assertIn('src="/theme-sync.js?v=3"', html)
    ...
    def test_consumption_page_and_assets_are_mounted_under_tuketim(self):
        ...
        self.assertIn('href="/tuketim/consumption.css?v=10"', html)
        self.assertIn('src="/tuketim/consumption.js?v=14"', html)
        ...
        self.assertIn('class="consumption-chart-forecast"', html)
        self.assertIn("TAHMİN ÖZETİ", html)
        self.assertIn("SABİTLENEN TAHMİN", html)
        self.assertIn("Saatlik Tüketim ve Tahmin", html)
        self.assertIn("Seçilen gün tahmini", html)
        self.assertNotIn("<h2>Saatlik Tüketim Tahmini</h2>", html)
        self.assertNotIn('id="consumptionForecastChart"', html)
        self.assertIn('id="consumption-table"', html)
        self.assertIn('data-suite-theme-toggle', html)
        self.assertIn('class="suite-menu-close"', html)
        self.assertIn('id="sidebar-lock"', html)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', html)
        self.assertIn('href="/module-suite.css?v=54"', html)
        self.assertIn('src="/module-suite.js?v=14"', html)
        self.assertIn(
        ...
        self.assertIn(".consumption-forecast-meta", css)
        self.assertIn(".consumption-chart-forecast", css)
        self.assertIn(".consumption-legend .forecast-today", css)
        ...
        self.assertIn("&targetDate=", script)
        self.assertIn("tomorrowForecast", script)
        self.assertIn("lockedForecastRange", script)
        self.assertIn("1 SAAT 10 DK ÖNCE SABİT", script)
        self.assertIn("chartColors.push(\"#ff7a1a\")", script)
        self.assertIn("legend: {show: false}", script)
        ...
        self.assertIn("CLIENT_CACHE_PREFIX", script)
        self.assertIn("cachedApi", script)
        self.assertIn("sessionStorage.setItem", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

    def test_system_direction_forecast_page_is_mounted(self):
        status, content, _ = self.get("/sistem-yonu-tahmini/")
        html = content.decode("utf-8")

        self.assertEqual(status, 200)
        self.assertIn("<title>Baha Sistem Yönü Tahmini</title>", html)
        self.assertIn('class="baha-suite-page baha-suite-sistem"', html)
        self.assertIn(
            'href="/sistem-yonu-tahmini/" class="suite-nav-forecast" aria-current="page"',
            html,
        )
        self.assertIn("Sistem Yönü Tahmini", html)
        self.assertIn('class="suite-sidebar"', html)
        self.assertIn('data-suite-scroll="#system-timeline"', html)
        self.assertIn('data-suite-scroll="#system-samples"', html)
        self.assertIn('data-suite-scroll="#system-validation"', html)
        self.assertIn('id="system-validation"', html)
        self.assertNotIn('class="validation-toolbar"', html)
        self.assertNotIn('id="validationDate"', html)
        self.assertNotIn('id="validationRefresh"', html)
        self.assertNotIn('id="validationNote"', html)
        self.assertIn('id="historicalForecastDate"', html)
        self.assertIn('id="historicalForecastPrev"', html)
        self.assertIn('id="historicalForecastNext"', html)
        self.assertIn('id="historicalForecastRun"', html)
        self.assertIn('id="historicalForecastLive"', html)
        self.assertLess(
            html.index("01 / SAATLİK TAHMİN"),
            html.index('id="validationMount"'),
        )
        self.assertIn("02 / TAHMİN DOĞRULAMA", html)
        self.assertIn("03 / OLASILIK", html)
        self.assertIn("04 / METOT", html)
        self.assertIn("05 / GEÇMİŞ ÖRNEKLER", html)
        ...
        self.assertIn('id="validationDailyQuality"', html)
        self.assertIn("TAHMİN BAŞARI LABORATUVARI", html)
        self.assertIn(">Gerçekleşen saat</span>", html)
        self.assertNotIn('class="observed" hidden><i>', html)
        self.assertIn('id="validationHourLatest"', html)
        self.assertIn('data-suite-user-email', html)
        self.assertIn('id="sidebar-lock"', html)
        self.assertIn('href="/module-suite.css?v=54"', html)
        self.assertIn('href="/system-direction-forecast.css?v=21"', html)
        self.assertIn('src="/module-suite.js?v=14"', html)
        self.assertIn('src="/system-direction-forecast.js?v=21"', html)
        ...
        status, content, headers = self.get("/system-direction-forecast.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn('/sistem-yonu-tahmini/api/validation', script)
        self.assertIn('params.set("includeValidation", "1")', script)
        self.assertIn("sessionStorage", script)
        self.assertIn("renderValidation", script)
        self.assertIn("renderValidationDirection", script)
        self.assertIn("renderQualityMetrics", script)
        self.assertIn("focusLatestValidationHour", script)
        self.assertIn("validation-direction-hour", script)
        self.assertIn("actual-lane", script)
        self.assertIn("forecast-lane", script)
        self.assertIn("activeValidationDate", script)
        self.assertNotIn('$(\"validationDate\")', script)
        self.assertNotIn('$(\"validationRefresh\")', script)
        self.assertIn('params.set("historical", "1")', script)
        self.assertIn("historicalForecastDate", script)
        self.assertIn("historicalForecastPrev", script)
        self.assertIn("historicalForecastNext", script)
        self.assertIn("shiftIsoDate", script)
        ...
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".validation-direction-track", css)
        ...
        self.assertIn(".history-date-nav", css)
        self.assertIn(".history-date-step", css)
        self.assertIn(
    ...
    def test_android_app_user_agent_receives_native_shell_styles(self):
        request = urllib.request.Request(
            self.base_url + "/piyasa/",
            headers={
                "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
        ...
        self.assertIn('href="/android-app.css?v=13"', html)
        self.assertIn('class="sidebar"', html)
        status, content, headers = self.get("/android-app.css?v=13")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".baha-suite-nav", css)
        self.assertIn(".suite-sidebar-overlay", css)
        self.assertIn(".baha-suite-piyasa .sidebar", css)
        self.assertIn(".baha-suite-sistem .suite-sidebar nav", css)
        self.assertIn("flex: 0 0 48px !important", css)
        ...
        for path in (
            ...
            with self.subTest(path=path):
                ...
                self.assertIn('href="/android-app.css?v=13"', module_html)
    ...
    def test_consumption_api_and_xlsx_use_shared_epias_session(self):
        ...
        APP._consumption_forecast = (
            lambda *args, **kwargs: forecast_calls.append((args, kwargs))
            or forecast_fixture
        )
        try:
            status, content, _ = self.get(
    ...
    def test_uretim_script_paths_are_mounted_under_uretim(self):
        ...
        self.assertIn('window.location.replace("/login")', script)
        self.assertNotIn('/uretim/>', script)
        ...
        self.assertIn("const currentMonthStart = new Date", script)
        self.assertIn("new Date(end.getFullYear(), end.getMonth(), 1, 12)", script)
        ...
        self.assertNotIn("EPİAŞ eksik veri", script)
        self.assertIn(
        ...
        self.assertNotIn("quickReportRange", script)
        self.assertNotIn("dailyReportButton", script)
    ...
    def test_uretim_page_uses_equalized_logo_size(self):
        ...
        self.assertIn('href="/module-suite.css?v=54"', html)
        self.assertIn("08 / SAATLİK DENGE", html)
        self.assertIn("Günlük saatlik denge özeti", html)
        self.assertIn('class="section-summary-copy"', html)
        self.assertNotIn("08 / SAATLIK ÖZET", html)
        self.assertNotIn("Saatlik UEVM − UEÇM özet", html)
        self.assertNotIn("09 / HIZLI RAPORLAR", html)
        self.assertNotIn('id="dailyReportButton"', html)
        self.assertNotIn('id="monthlyReportButton"', html)
        self.assertNotIn('data-range="30"', html)
        self.assertNotIn("Son 30 g\u00fcn", html)
        self.assertIn('src="/module-suite.js?v=14"', html)
        self.assertIn('src="/theme-sync.js?v=3"', html)
        self.assertIn('src="/uretim/app.js?v=15"', html)
        ...
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("DASHBOARD_CACHE_KEY", script)
        self.assertIn("readDashboardCache", script)
        self.assertIn("sessionStorage.setItem", script)
        ...
        self.assertIn(".baha-suite-nav a.suite-nav-forecast", css)
        self.assertNotIn(".suite-nav-short", css)
        ...
        self.assertEqual(status, 200)
        self.assertEqual(headers.get_content_type(), "image/png")
        self.assertGreater(len(content), 60_000)
        self.assertLess(len(content), 120_000)
    ...
    def test_shared_module_theme_assets_are_served(self):
        ...
        self.assertIn(".baha-suite-sistem .suite-sidebar *", css)
        self.assertIn("flex: 0 0 48px !important", css)
        self.assertIn("overflow: hidden !important", css)
        self.assertIn(".suite-sidebar-lock-button", css)
        self.assertIn(".sidebar-lock-button", css)
        self.assertIn(
            "@media (min-width: 1025px) and (hover: hover) and (pointer: fine)",
            css,
        )
        self.assertIn("body.suite-sidebar-pinned .suite-sidebar-lock-button", css)
        self.assertIn("body.suite-sidebar-pinned .sidebar-lock-button", css)
        ...
        self.assertIn(".baha-suite-baraj .baraj-map-legend", css)
        self.assertIn(".baha-suite-baraj .baraj-basin-feature.active", css)
        self.assertIn(".baha-suite-baraj .baraj-map-legend .fullness-low", css)
        self.assertIn(".baha-suite-baraj .baraj-map-legend .fullness-mid", css)
        self.assertIn(".baha-suite-baraj .baraj-map-legend .fullness-high", css)
        self.assertIn(".baha-suite-baraj .baraj-basin-feature.fullness-unknown", css)
        self.assertIn("fill: var(--basin-fill", css)
        ...
        self.assertIn("setDesktopSidebarHover", script)
        self.assertIn("setSidebarPinned", script)
        self.assertIn("#sidebar-lock", script)
        self.assertIn("desktopLock", script)
        self.assertIn("canPinSidebar", script)
        self.assertIn(
            'window.matchMedia("(min-width: 1025px) and (hover: hover) and (pointer: fine)")',
            script,
        )
        ...
        self.assertIn("suppressUretimWarning", script)
        ...
        self.assertIn("connectRoutePrefetching", script)
        self.assertIn('prefetch.rel = "prefetch"', script)
        self.assertNotIn("window.setTimeout(() => window.location.assign(target.href), 190)", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")
    ...
    def test_tv_mode_and_command_center_assets_are_served(self):
        ...
        self.assertIn('src="tv.js?v=5"', html)
        self.assertNotIn('<strong id="tvDeviation">—</strong><small>%</small>', html)
        self.assertNotIn('<strong id="tvDamAverage">—</strong><small>%</small>', html)
        ...
        script = content.decode("utf-8")
        self.assertIn('setText("tvDeviation", fmtPercent(', script)
        self.assertIn('setText("tvDamAverage", Number.isFinite', script)
        self.assertEqual(status, 200)
        ...
        self.assertIn("marketFreshness.smfIncomplete", script)
        self.assertIn("scheduleDataRefresh(data)", script)
        self.assertIn("row.smfPublished === false ? null : row.smf", script)
        self.assertIn('addEventListener("visibilitychange"', script)
        self.assertIn("DATA_REUSE_MS = 60000", script)
        self.assertIn("sessionStorage.setItem", script)
        self.assertIn("readCachedReport", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")
