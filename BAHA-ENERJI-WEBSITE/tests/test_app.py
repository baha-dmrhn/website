...
class SuiteHttpTests(unittest.TestCase):
    ...
    def test_login_is_a_clean_shared_portal_page(self):
        ...
        self.assertIn("Panel hazırlanıyor...", html)
        self.assertIn('src="/suite-loading.js?v=1"', html)
        ...
        self.assertEqual(status, 200)
        self.assertIn("baha-enerji-shell-v16", worker)
        ...
        self.assertEqual(status, 200)
        self.assertIn("Panel hazırlanıyor...", loading_html)
        self.assertIn('src="/panel-loading.js"', loading_html)
    ...
    def test_android_app_user_agent_receives_native_shell_styles(self):
        request = urllib.request.Request(
            ...
            headers={
                "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
                "User-Agent": "Mozilla/5.0 BahaEnerjiAndroid/2.0",
                "Accept": "text/html",
            },
        )
        with urllib.request.urlopen(request, timeout=5) as response:
        ...
        self.assertIn('href="/android-app.css?v=1"', html)
        status, content, headers = self.get("/android-app.css?v=1")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
