...
def command_center_payload() -> dict:
    ...
    next_day = next_day_payload()
    dams = reservoir_payload()
    production = production_payload()
    consumption = consumption_payload()
    dam_items = dams["items"]
    highest = max(dam_items, key=lambda item: item["activeFullnessAmount"])
    lowest = min(dam_items, key=lambda item: item["activeFullnessAmount"])
    return {
        "date": TODAY,
        "generatedAt": "2026-07-20T14:00:00+03:00",
        "modules": {
        ...
        "damSummary": {
            "date": TODAY,
            "count": len(dam_items),
            "average": sum(item["activeFullnessAmount"] for item in dam_items) / len(dam_items),
            "highest": {"name": highest["dam"], "value": highest["activeFullnessAmount"]},
            "lowest": {"name": lowest["dam"], "value": lowest["activeFullnessAmount"]},
            "source": "EPÄ°AÅ",
        },
        "errors": {},
...
class VisualHandler(APP.RequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        fixtures = {
            ...
            "/api/command-center": command_center_payload,
        }
...
def run_browser(command: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    ...
    # Chromium child processes can inherit stdout/stderr handles on Windows and
    # keep communicate() waiting after the screenshot has already been written.
    ...
    try:
        ...
        try:
            ...
            process.kill()
        return subprocess.CompletedProcess(command, 124, "", "Tarayici zaman asimina ugradi.")
...
def main() -> int:
    ...
    parser.add_argument("--scenario", help="Run only the named visual scenario")
    ...
    failures = []
    try:
        scenarios = [item for item in SCENARIOS if not args.scenario or item[0] == args.scenario]
