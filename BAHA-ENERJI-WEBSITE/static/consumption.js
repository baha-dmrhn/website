(() => {
  ...
  async function loadData(force = false) {
    ...
    try {
      ...
      const [comparison, forecast, tomorrowForecast] = await Promise.all([
        compare
        ...
        api(forecastUrl(date, date, refresh)).catch(() => {
          forecastFailed = true;
          return null;
        }),
        api(forecastUrl(date, tomorrowDate, refresh)).catch(() => {
  ...
  const consumptionDownloadMenu = document.querySelector('[data-download-menu="consumption"]');
  const consumptionExportPaths = {
    xlsx: "/api/export.xlsx?date=",
    pdf: "/api/export.pdf?date=",
    csv: "/api/export.csv?date=",
  };
  function closeConsumptionDownloadMenu() {
    consumptionDownloadMenu?.classList.remove("open");
    $("consumptionXlsx").setAttribute("aria-expanded", "false");
  }
  $("consumptionXlsx").addEventListener("click", (event) => {
    event.stopPropagation();
    if (!state.data?.summary?.availableHours) return;
    const open = !consumptionDownloadMenu?.classList.contains("open");
    consumptionDownloadMenu?.classList.toggle("open", open);
    $("consumptionXlsx").setAttribute("aria-expanded", String(open));
  });
  consumptionDownloadMenu?.querySelectorAll("[data-download-format]").forEach((button) => {
    button.addEventListener("click", (event) => {
      event.stopPropagation();
      if (!state.data?.summary?.availableHours) return;
      const format = button.dataset.downloadFormat || "xlsx";
      closeConsumptionDownloadMenu();
      window.location.href = `${consumptionExportPaths[format] || consumptionExportPaths.xlsx}${encodeURIComponent(state.data.date)}`;
    });
  });
  document.addEventListener("click", closeConsumptionDownloadMenu);
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") closeConsumptionDownloadMenu();
  });
